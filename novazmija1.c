...
/* ================= SOUND ================= */
void playNokiaTune() {
    Beep(659, 150); Beep(587, 150); Beep(370, 300); Beep(415, 300);
}
...
void drawGame() {
    gotoxy(0, 0);
    setColor(11);
    printf("SNAKE | Score: %04d | Lives: %d | P Pause | M Menu | R Restart",
        score, lives);
...
void showHighscore() {
    ...
    while (fgets(line, 255, f)) {
        char ime[50], s[10], t[10], l[10], r[50];
        sscanf(line,
...
void updateGame() {
    ...
    if (snake[0].x == food.x && snake[0].y == food.y) {
        ...
        if (snakeLen < MAX_SNAKE) {
            snake[snakeLen] = snake[snakeLen - 1];  // 🔧 KLJUČNI FIX
            snakeLen++;
        }
